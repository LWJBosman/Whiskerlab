# -*- coding: utf-8 -*-
"""
Created on Tue Nov 29 11:20:07 2022
...
for ss in range(len(group1)):
    ...
    max_runtime = 150000   # maximum runtime
...
with pd.ExcelWriter("C:\\LabProject\\LadderProject\\Data\\excel\\sorted2\\alltouches"+group+"_.xlsx", engine='xlsxwriter') as writer:
        test.to_excel(writer, sheet_name = 'Touches')
df_types = pd.DataFrame(types)
with pd.ExcelWriter("C:\\LabProject\\LadderProject\\Data\\excel\\sorted2\\runtypes"+group+".xlsx", engine='xlsxwriter') as writer:
      df_types.to_excel(writer, sheet_name = 'Touches')

#%% output files          
# # save file with parameters analysis
df_analysis.to_excel("C:\\Labproject\\LadderProject\\Data\\excel\\sorted2\\Parameters"+group+".xlsx", index = False)
...
skipped.to_excel("C:\\Labproject\\LadderProject\\Data\\excel\\sorted2\\skippedruns"+group+"_.xlsx", index = False)
w_sequence = pd.concat(skip_sequence)
w_sequence.to_excel("C:\\Labproject\\LadderProject\\Data\\excel\\sorted2\\Wrong_sequence_runs"+group+"_.xlsx", index = False)
# save front steps
df_front.to_excel("C:\\LabProject\\LadderProject\\Data\\excel\\sorted2\\Steps_front_"+group+"_.xlsx", index = False)
# save lag and support all runs
supports = pd.concat(all_support, axis = 1).T
supports.to_excel("C:\\LabProject\\LadderProject\\Data\\excel\\sorted2\\Support_all_runs"+group+".xlsx", index = False)
lags = pd.DataFrame(all_lag, columns = ['session id', 'subject id', 'run id', 'ses nr', 'lag', 'pair'])
lags.to_excel("C:\\LabProject\\LadderProject\\Data\\excel\\sorted2\\Lag"+group+".xlsx", index = False)
# save overlap
overlaps = pd.DataFrame(all_overlap, columns = ['session id', 'run id', 'ses nr', 'overlap', 'pair'])
overlaps.to_excel("C:\\LabProject\\LadderProject\\Data\\excel\\sorted2\\Overlap_1run"+group+".xlsx", index = False)
# save stats touches
df_stats_touches.to_excel("C:\\LabProject\\LadderProject\\Data\\excel\\sorted2\\Stats_touches"+group+"_.xlsx", index = False)