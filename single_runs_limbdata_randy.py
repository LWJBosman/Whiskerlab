# -*- coding: utf-8 -*-
"""
Created on Fri Sep 23 14:26:32 2022
...
# all values are strings -> change all columns except statesequence to numeric values
...
# mean rntime of the ouse groups is not equal/similair in all groups wo we should the mean of each 
# group to make a more personalized. 
group = 'SCA1MUT'
if group == 'ercc1_mut':
...
# gait_cycle_und = pd.DataFrame(0, index=['undetermined'], columns= range(time_begin, time_end + 1))
...
# empty lists that will be filled with the touch ids of touches assigned to that paw
lf = []
lh = []
rf = []
rh = []
...
for ru in range(len(rung_list)):
    df_multitouch = df_touches[df_touches["rung"] == rung_list[ru]]
    n = len(df_multitouch)
...
#%% limb detection
# give 2 touches on same rung, same side priority
# assign first touch to front paw, second touch to hind paw


left = df_touches[df_touches["side"] == 0]
right = df_touches[df_touches["side"] == 1]
...
for d in range(len(double_rungs_left)):
    df_doubletouch = left[left["rung"] == double_rungs_left[d]]
...
for d in range(len(double_rungs_right)):
    df_doubletouch = right[right["rung"] == double_rungs_right[d]]
    ...
    if left["rung"].iloc[ns-1] not in double_rungs_left and left["rung"].iloc[ns] not in double_rungs_left:
        ...
        # if left["begin touch"].iloc[ns] < left["end touch"].iloc[ns-1]:
            ...
            if left["touch"].iloc[ns-1] > split_time*mean:
                ...
                if left["direction"].iloc[ns] == 0:
                    if left["rung"].iloc[ns] < left["rung"].iloc[ns-1]:
                        # rungs were changed to be from 1 to 37 during calculation -> normally step behind will have higher rung number
                        # lower rung than previous touch
                        # consider both touches as single touches -> no split
...
# something causes the touches to get doubled up 
...
for xx in range(len(df_touches)):
    ...
    # elif df_touches["touch id"].iloc[xx] in undetermined:
    #     gait_cycle_und.loc["undetermined", range(int(df_touches["begin touch"].iloc[xx]), int(df_touches["end touch"].iloc[xx]) + 1)] = 1
    #     df_touches["paw"].iloc[xx] = 'undetermined'
    #%%
...
for ph in range(len(step_lh)):
    ...
    total = swing + stance
    inbetween = (swing/total)*100
    swing_phase_lh.append(inbetween)                    # swing phase %
...
with pd.ExcelWriter("C:\\LabProject\\LadderProject\\Data\\excel\\VA\\runs\\Data_run_"+str(run)+".xlsx", engine='xlsxwriter') as writer:
     df_touches.to_excel(writer, sheet_name = 'Touches')
     # df_analysis.to_excel(writer, sheet_name = 'analysis')
...
output_file(filename="C:\\LabProject\\LadderProject\\Data\\Figures\\VA\\Gait_pattern_run_"+str(run)+".html", title="Gait pattern run %s" %run)
...
if (df_touches["direction"] == 1).all():
    ...
    fig.y_range = Range1d(1, 37)
    fig1.y_range = Range1d(1, 37)
    ...
    fig.y_range = Range1d(37, 1)
    fig1.y_range.flipped = True
    fig1.y_range = Range1d(37, 1)
...
gait_cycle2 = gait_cycle.copy()
gait_cycle3 = gait_cycle.copy()
gait_cycle4 = gait_cycle.copy()

gait_cycle.loc["rf"] = float('nan')
gait_cycle.loc["rh"] = float('nan')
gait_cycle.loc["lh"] = float('nan')

gait_cycle2.loc["rf"] = float('nan')
gait_cycle2.loc["lf"] = float('nan')
gait_cycle2.loc["lh"] = float('nan')

gait_cycle3.loc["rh"] = float('nan')
gait_cycle3.loc["lf"] = float('nan')
gait_cycle3.loc["lh"] = float('nan')

gait_cycle4.loc["rf"] = float('nan')
gait_cycle4.loc["rh"] = float('nan')
gait_cycle4.loc["lf"] = float('nan')

sns.heatmap(gait_cycle, cbar = False, xticklabels = tick, cmap = ['white', 'deepskyblue', 'darkorange', 'navy', 'firebrick'],linewidths=0)
sns.heatmap(gait_cycle2, cbar = False, xticklabels = tick, cmap = ['white', 'firebrick', 'darkorange', 'navy', 'deepskyblue'],linewidths=0)
sns.heatmap(gait_cycle3, cbar = False, xticklabels = tick, cmap = ['white', 'deepskyblue', 'darkorange', 'firebrick', 'navy'],linewidths=0)
sns.heatmap(gait_cycle4, cbar = False, xticklabels = tick, cmap = ['white', 'firebrick', 'deepskyblue', 'navy', 'darkorange'],linewidths=0)


plt.title("Gait phase")
plt.savefig("C:\\LabProject\\LadderProject\\Data\\Figures\\"+str(run)+"color.eps")