# -*- coding: utf-8 -*-
"""
Created on Wed Apr 27 10:35:14 2022
...
db_filepath = "C:\LabProject\LadderProject\Data\Data_Videos\ErasmusLadder28-2-23.sdf"
db_passw = "mypassword"
...
Balbc = session_group["Balbc"].dropna()
Bl6 = session_group["Bl6"].dropna()
FVB = session_group["FVB_dick"].dropna()
...
# empty_mut = [1820, 1823, 1825, 1828, 1830, 1832]
# empty_ctr = [1821, 1824, 1826, 1827, 1829, 1831]
# ercc1_mut = ercc1_mut[~ercc1_mut.isin(empty_mut)].reset_index(drop=True)
# ercc1_ctr = ercc1_ctr[~ercc1_ctr.isin(empty_ctr)].reset_index(drop=True)
...
column_analysis = ['session id', 'run id', 'session nr', 'direction', 'time run', 'number of steps', 'even steps', 'odd steps', 'step 2', 'step 4', 'step 6', 'swing phase lf', 'swing phase lh', 'swing phase rf', 'swing phase rh', 'stance phase lf', 'stance phase lh', 'stance phase rf', 'stance phase rh', 'lag lf/rh', 'lag rf/lh', 'support diagonal', 'support girdle', 'support lateral', 'support 0 paws', 'support 1 paw', 'support 3 paws', 'support 4 paws', 'high rungs front', 'high rungs hind', 'low rungs front', 'low rungs hind']
df_analysis = pd.DataFrame(columns = column_analysis)
column_front = ['session id', 'run id', 'session nr', 'direction', 'total steps', 'h-h 2', 'h-h 4', 'h-h other', 'h-l', 'l-h', 'l-l']
...
for ss in range(len(group1)):
    ...
    min_runtime = 800       # minimum runtime
    max_runtime = 15000     # maximum runtime
    session = group1[ss]     # session id of experiment
    query = "select trial.id, statesequence, trialtype, session_id, state, state_duration, sessionnr from trial join state on trial.id = state.trial_id join session on trial.session_id = session.id where state = 6 and state_duration > %s and state_duration < %s and session_id = %s;" %(min_runtime, max_runtime, session)
    data = db.query(query)
    column = ["run id", "state sequence", "runtype", "session id", "state", "state time", "session nr"]
    df_runs = pd.DataFrame(np.column_stack(data), columns = column)
    # only use runs with runsequence 1-3-6 and 1-3-4-6
    df_sequence1 = df_runs[df_runs["state sequence"] == '1-3-6']
    
    df_sequence2 = df_runs[df_runs["state sequence"] == '1-3-4-6']
    list_runs = list(df_sequence1["run id"]) + list(df_sequence2["run id"])
    ...
    for a in range(len(list_runs)):
        ...
        run = list_runs[a]
        query = "select side, rung, touch_begin, touch_end, trial.id as trial_id, trialnr, touch.id as touch_id, state, state_duration, directiontoend, session_id, statesequence, sessionnr from trial join touch on trial.id = touch.trial_id join state on trial.id = state.trial_id join session on trial.session_id = session.id where state = 6  and trial.id = %s;" %run
        data = db.query(query)
        
        column = ["side", "rung", "begin touch", "end touch", "run id", "run number", "touch id", "state", "state duration", "direction", "session id", "statesequence", "session nr"]
        ...
        if (df["direction"] == 0).all(): # 37 -> 1
            ...
            for s in range(len(df)):
                if df["side"].iloc[s] == 0:
                    ...
                    df["side"].iloc[s] = 0
        #%% filter touches
        if (df["touch"] > 1600).any():
            skip_run.append([run, 'long touch', session])
            continue
        else:
            "Filter the collected data: minimum and maximum duration of a touch"
            "We put the minimum at 30 because of the trough in the touch histogram, the maximum is a bit more arbitrary and depends on what you want to include and exclude in your research"
            touch_min = 31         # minimum touch time [ms]
            ...
            time_begin = min(df_touches["begin touch"])             # time first touch
            time_end = max(df_touches["end touch"])                 # time end last touch
            ...
            #df_analysis["time run"].iloc[-1] = df["state duration"].iloc[0]
            df_analysis["time run"].iloc[-1] = max(df_touches["end touch"]) - min(df_touches["begin touch"])
            ...
            #     all_lag.append([session, run, df["session nr"].iloc[0], lf_rh["lag"].iloc[uu], 'lf/rh'])
            # if len(lf_rh) == 0:
            ...
            #     all_lag.append([session, run, df["session nr"].iloc[0], rf_lh["lag"].iloc[uu], 'rf/lh'])
            # if len(rf_lh) == 0:
            ...
            for ii in range(len(df_lf)-1):
                ...
                for jj in range(len(df_rh)):
                    ...
                    if zeropi <= tar <= pi:
                        ...
                        all_lag.append([session, run, df["session nr"].iloc[0], phase, 'lf/rh'])
                    elif pi < tar < twopi:
                        # swing phase ref
                        x = twopi - pi
                        xtar = tar - pi
            ...
            for ii in range(len(df_rf)-1):
                ...
                for jj in range(len(df_lh)):
                    ...
                    if zeropi <= tar <= pi:
                        ...
                        all_lag.append([session, run, df["session nr"].iloc[0], phase, 'rf/lh'])
                    elif pi < tar < twopi:
                        x = twopi - pi
                        xtar = tar - pi
                        phase = (xtar/x) + 1
            ...
            # with pd.ExcelWriter("D:\\Nanobiologie\\Master\\MEP\\Data\\Figures\\Figure 5\\"+group+"\\Data_run_"+str(run)+".xlsx", engine='xlsxwriter') as writer:
            ...
            # output_file(filename="D:\\Nanobiologie\\Master\\MEP\\Data\\"+group+"\\Runs\\Gait_pattern_run_"+str(run)+".html", title="Gait pattern run %s" %run)
            ...
            # plt.savefig("D:\\Nanobiologie\\Master\\MEP\\Data\\"+group+"\\Runs\\Gait_phase_run_"+str(run)+".png")
            # plt.clf()
            ...
            # plt.savefig("D:\\Nanobiologie\\Master\\MEP\\Data\\"+group+"\\Runs\\Support_pattern_run_"+str(run)+".png")
            # plt.clf()
            # # plot lag phase
            # r = np.full((len(lag_lf_rh),), 1)
            # r2 = np.full((len(lag_rf_lh),), 2)
            # lf_rh = [(x/100)*2*np.pi for x in lag_lf_rh]
            # rf_lh = [(x/100)*2*np.pi for x in lag_rf_lh]
            # fig = plt.figure()
            ...
            # fig.savefig("D:\\Nanobiologie\\Master\\MEP\\Data\\"+group+"\\Runs\\Lag_phase_run_"+str(run)+".png", bbox_inches = 'tight')
            # fig.clf()
            # plt.close('all')
...
with pd.ExcelWriter("C:\\LabProject\\LadderProject\\Data\\excel\\alltouches"+group+"_.xlsx", engine='xlsxwriter') as writer:
       df_alltouches.to_excel(writer, sheet_name = 'Touches')
# # save file with parameters analysis
df_analysis.to_excel("C:\\Labproject\\LadderProject\\Data\\excel\\Parameters_3.0"+group+".xlsx", index = False)
# df_analysis.to_excel("D:\\Nanobiologie\\Master\\MEP\\Data\\Figures\\Figure 6\\Parameters_"+group+".xlsx", index = False)
# # save list with runs skipped
skipped = pd.DataFrame(skip_run, columns = ['run id', 'reason', 'session id'])
skipped.to_excel("C:\\Labproject\\LadderProject\\Data\\excel\\skippedruns"+group+".xlsx", index = False)
# w_sequence = pd.concat(skip_sequence)
# w_sequence.to_excel("C:\Labproject\LadderProject\Data\excel\Wrong_sequence_runs"+group+".xlsx", index = False)
# # save front steps
# df_front.to_excel("D:\\Nanobiologie\\Master\\MEP\\Data\\Figures\\Figure 4\\Steps_front_"+group+".xlsx", index = False)
# # save lag and support all runs
# supports = pd.concat(all_support, axis = 1).T
# supports.to_excel("D:\\Nanobiologie\\Master\\MEP\\Data\\Figures\\Figure 5\\Support_all_runs_"+group+".xlsx", index = False)
# lags = pd.DataFrame(all_lag, columns = ['session id', 'run id', 'ses nr', 'lag', 'pair'])
# lags.to_excel("C:\\LabProject\\LadderProject\\Data\\Figures\\Figure 5\\Lag_all_runs_"+group+".xlsx", index = False)
# # save overlap
# overlaps = pd.DataFrame(all_overlap, columns = ['session id', 'run id', 'ses nr', 'overlap', 'pair'])
# overlaps.to_excel("D:\\Nanobiologie\\Master\\MEP\\Data\\Figures\\Figure 5\\Overlap_all_runs_"+group+".xlsx", index = False)
# save stats touches
# df_stats_touches.to_excel("C:\LabProject\LadderProject\Data\excel\Stats_touches_run"+group+".xlsx", index = False)